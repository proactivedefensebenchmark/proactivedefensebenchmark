"""
Batch Anti-Forgery Lab Attack on Multiple Wrappers
Processes images using Lab color space perturbation
"""
import torch
...
def main(args):
    ...
    if args.output_base is None:
        args.output_base = os.path.join(project_root, "batch_results_anti")
    ...
    for wrapper_name in args.wrappers:
        ...
        output_dir = os.path.join(output_root, f"result_{wrapper_name}_anti")
        os.makedirs(output_dir, exist_ok=True)

        # Pre-check: skip wrapper entirely if all images are already done
        remaining = [
            (idx, img_path) for idx, img_path in enumerate(img_paths)
            if not os.path.exists(os.path.join(output_dir, f"img_{idx+1:03d}", "x_adv.pt"))
        ]
        if not remaining:
            print(f"⏭️  {wrapper_name}: all {len(img_paths)} images already done, skipping")
            continue
        print(f"   {len(img_paths) - len(remaining)} / {len(img_paths)} already done, processing {len(remaining)} remaining")
        ...
        # Process images
        for idx, img_path in tqdm(remaining, desc=wrapper_name):

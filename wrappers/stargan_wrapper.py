...
stargan_path = os.path.join(project_root, "deepfake_generators", "StarGAN")

if not os.path.exists(stargan_path):
    raise FileNotFoundError(f"StarGAN path not found: {stargan_path}")

if stargan_path not in sys.path:
    sys.path.append(stargan_path)
...
class StarGANWrapper(nn.Module):    
    ...
    def __init__(
        ...
        if model_save_dir is None:
            model_save_dir = os.path.join(stargan_path, f'stargan_celeba_{image_size}', 'models')
        ...
        G_path = os.path.join(self.model_save_dir, f'{test_iters}-G.ckpt')
        if os.path.exists(G_path):
            self.G.load_state_dict(torch.load(G_path, map_location='cpu'))
            print(f"Loaded StarGAN Generator from {G_path}")
        else:
        ...
        if attr_path is None:
            attr_path = os.path.join(stargan_path, 'data', 'celeba', 'list_attr_celeba.txt')
        ...
        # else:
        self.default_c_org = torch.FloatTensor([0, 0, 0, 0, 0])
    ...
    def decode(self, x: torch.Tensor, target_attr=None, c_org=None, ref=None):
        batch_size = x.size(0)
        ...
        return self.G(x, c_trg)

    def forward(self, x: torch.Tensor, target_attr, c_org=None):
        self._cached_input = x.clone()
        return self.decode(x, target_attr, c_org)

    def transform_with_label(self, x: torch.Tensor, c_trg: torch.Tensor):
